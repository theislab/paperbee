...
def search_articles_command_callback(
    command, ack: Ack, respond: Respond, logger: Logger, client, context
):
    """
    ...
    try:
        ...
        if not user_input:
            ...
            ack(
                "Received your command! Searching for papers... This might take a while."
            )
        ...
        if "," in user_input:
            ...
            if len(parts) == 2:
                ...
                if not is_valid_date(user_date):
                    respond(
                        f"The date must be in YYYY-MM-DD format. You entered: '{user_date}'"
                    )
                    return
                user_query, user_query_valid = is_valid_query(user_query)
                if not user_query_valid:
                    respond(
                        f"The query must be in the format '[keyword] OR [keyword]...'. You entered: '{user_query}'"
                    )
                    return
                finder = PapersFinder(
                ...
                respond(
                    "Format incorrect. Please use the format 'query, date' or 'query' if no date is provided."
                )
                return
        elif user_input:
            ...
            if not user_query_valid:
                respond(
                    f"The query must be in the format '[keyword] OR [keyword]...'. You entered: '{user_input}'"
                )
                return
            finder = PapersFinder(
                root_dir="/home/daniele/Code/slack-papers-app/files/",
            ...
            respond(
                f"Format incorrect. Use the format 'query,date' or 'query'. You entered: '{user_input}"
            )
            return
...
def is_valid_query(query):
    ...
    if all(part.startswith("[") and part.endswith("]") for part in parts):
        valid_structure = all(
            part.count("[") == 1 and part.count("]") == 1 for part in parts
        )
